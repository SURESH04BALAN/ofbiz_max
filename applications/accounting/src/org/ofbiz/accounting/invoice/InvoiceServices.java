...
public class InvoiceServices {
    ...
    //            try {
    //                throw new Exception("createInvoiceForOrderAllItems");
    //            }
    //            catch(Exception e){
    ...
    //            }
        ...
        Debug.logInfo("createInvoiceForOrder orderId: " + orderId, module);
        ...
        try {
            ...
            for (GenericValue paymentPref : orderPaymentPrefs) {
                ...
                Debug.logInfo("Payment count: " + payments.size(),module);
            }
            ...
            for (GenericValue payment : currentPayments) {
                ...
                if (notApplied.signum() > 0) {
                    Debug.logInfo("notApplied.signum(): " + notApplied,module);
                    Debug.logInfo("paymentId : " + payment.get("paymentId"),module);                    
    ...
    public static Map<String, Object> createInvoicesFromShipments(DispatchContext dctx, Map<String, ? extends Object> context) {
        ...
        Debug.logInfo("createInvoicesFromShipments start", module);
        ...
        for (String tmpShipmentId : shipmentIds) {
            try {
                ...
                Debug.logInfo("shipmentTypeId " + shipment.getString("shipmentTypeId"), module);
...
Debug.logInfo("createInvoicesFromShipments start - 1", module);
        ...
        try {
            if (purchaseShipmentFound) {
                ...
                while (itemsIter.hasNext()) {
                    ...
                    Debug.logInfo("ShipmentReceipt id" +  item.getString("receiptId"), module);
                    ...
                    if (UtilValidate.isEmpty(ownerPartyRole)) {
                        itemsIter.remove();
                        Debug.logInfo(" owner party role remove - 1", module);                                        
                    }
                }
                Debug.logInfo(" owner party role items: " + items.size(), module);                
...
Debug.logInfo("createInvoicesFromShipments start - 2", module);
        // group items by order
        Map<String, List<GenericValue>> shippedOrderItems = FastMap.newInstance();
Debug.logInfo("createInvoicesFromShipments start - 2.01.01", module);        
        for (GenericValue item : items) {
Debug.logInfo("createInvoicesFromShipments start - 2.01.02", module);                                
            String orderId = item.getString("orderId");
Debug.logInfo("createInvoicesFromShipments start - 2.01.03 id " + orderId, module);                                
            String orderItemSeqId = item.getString("orderItemSeqId");
    Debug.logInfo("createInvoicesFromShipments start - 2.01.03 orderItemSeqId " + orderItemSeqId, module);                                
            List<GenericValue> itemsByOrder = shippedOrderItems.get(orderId);
            if (itemsByOrder == null) {
                itemsByOrder = FastList.newInstance();
            }
Debug.logInfo("createInvoicesFromShipments start - 2.1", module);
...
Debug.logInfo("createInvoicesFromShipments start - 2.2", module);
...
Debug.logInfo("createInvoicesFromShipments start - 2.3", module);            
...
Debug.logInfo("createInvoicesFromShipments start - 2.4", module);
            // if none found, then okay to bill
            if (itemBillings.size() == 0) {
                itemsByOrder.add(item);
            }
Debug.logInfo("createInvoicesFromShipments start - 2.5", module);
            // update the map with modified list
            shippedOrderItems.put(orderId, itemsByOrder);
        }
Debug.logInfo("createInvoicesFromShipments start - 3", module);
...
Debug.logInfo("createInvoicesFromShipments start - 4", module);
...
Debug.logInfo("createInvoicesFromShipments start - 5", module);
...
Debug.logInfo("createInvoicesFromShipments start - 6", module);
...
Debug.logInfo("createInvoicesFromShipments start - 7", module);
        Map<String, Object> response = ServiceUtil.returnSuccess();
        response.put("invoicesCreated", invoicesCreated);
        return response;
    }
