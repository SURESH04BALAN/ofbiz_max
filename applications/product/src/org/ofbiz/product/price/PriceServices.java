...
public class PriceServices {
    ...
    public static Map<String, Object> calculateProductPrice(DispatchContext dctx, Map<String, ? extends Object> context) {
        ...
        // if currencyUomId is null get from properties file, if nothing there assume AUD (AUD: American Dollar) for now
        String currencyDefaultUomId = (String) context.get("currencyUomId");
        Debug.logInfo("currencyDefaultUomId  " + currencyDefaultUomId , module);
        String currencyUomIdTo = (String) context.get("currencyUomIdTo"); 
        if (UtilValidate.isEmpty(currencyDefaultUomId)) {
            currencyDefaultUomId =  EntityUtilProperties.getPropertyValue("general", "currency.uom.id.default", "AUD", delegator);
        }
    ...
    private static GenericValue getPriceValueForType(String productPriceTypeId, List<GenericValue> productPriceList, List<GenericValue> secondaryPriceList) {
        ...
        if(priceValue==null){
          if (Debug.infoOn()) Debug.logInfo("There is NO pricevalue " + productPriceTypeId , module);  
        }
        else{
            if (Debug.infoOn()) Debug.logInfo("There is pricevalue " + productPriceTypeId + " with the currencyUomId " + priceValue.getString("currencyUomId") + " and productId " + priceValue.getString("productId") + ", using the latest found with price: " + priceValue.getBigDecimal("price"), module);  
        }
        return priceValue;
    }
