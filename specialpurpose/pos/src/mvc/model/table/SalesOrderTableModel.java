 ...
 *
 */
public class SalesOrderTableModel extends AbstractTableModel {

    /**
     *
     */
    public static final String module = SalesOrderTableModel.class.getName();
    ...
    public enum Columns {
        ...
        ORDER_QTY_INDEX(3, decimalWidth, BigDecimal.class, "Order Qty", true),
        ORDER_PRICE_INDEX(4, decimalWidth, BigDecimal.class, "Price", true),
        ORDER_ONHAND_QTY_INDEX(5, decimalWidth, BigDecimal.class, "On Hand Qty", false),
        ORDER_ATP_QTY_INDEX(6, decimalWidth, BigDecimal.class, "Ava. To Prom.", false),
        ORDER_LIST_PRICE_INDEX(7, decimalWidth, BigDecimal.class, "List Price", false),
        ...
        ORDER_IS_PROMO(14, decimalWidth, String.class, "Is Promo", true);
...
//"Date 	OrderItemComposite Nbr 	OrderItemComposite Name 	OrderItemComposite Type 	Bill From Party 	Bill To Party 	Product Store 	Amount 	Tracking Code 	ORDER_PROD_INTERNALNAME_INDEX";
    ...
    public final void setListModel(ListModel<OrderItemComposite> listModel) {
        ...
        //Debug.logInfo("listModel.size: " + listModel.getSize(), module);
...
//                Debug.logInfo("record.getOrderItem().getitemDescription(): " + record.getOrderItem().getitemDescription(), "module");                    
            ...
            case ORDER_IS_PROMO:
                return record.getOrderItem().getisPromo();
...
//                return record.getOrderItem().getunitListPrice();                
            ...
            case ORDER_GST_PRICE_INDEX: {
                Debug.logInfo("Gst", module);
                if (UtilValidate.isNotEmpty(record.getOrderItem().getproductId())) {
                       Debug.logInfo("Gst record.getOrderItem().getproductId(" + record.getOrderItem().getproductId(), module);
                    try {
                        Product prod = ProductSingleton.getProduct(record.getOrderItem().getproductId());
                        Debug.logInfo("Gst taxable:  " + prod.gettaxable(), module);
                        if("Y".equals(prod.gettaxable())){
                            record.setGstAmount(BigDecimal.valueOf(0.1).multiply(record.getShoppingCartItem().getItemSubTotal()));
                        }
                        Debug.logInfo("Gst amount:  " + record.getGstAmount(), module);
                        return record.getGstAmount();
                    } catch (Exception ex) {
                        Logger.getLogger(SalesOrderTableModel.class.getName()).log(Level.SEVERE, null, ex);
                    }
                } else {
                     Debug.logInfo("no Gst amount:  " + record.getGstAmount(), module);
                    return record.getGstAmount();
                }
            }
...
//                    fireTableCellUpdated(rowIndex, ORDER_LINETOTAL_INDEX);
            ...
            case ORDER_LIST_PRICE_INDEX:
                ...
                break;
            case ORDER_IS_PROMO:
                record.getOrderItem().setisPromo((String) value);
                break;
            case ORDER_LASTPRICE_INDEX:
