...
abstract class RowTableModel<T> extends AbstractTableModel {
    ...
    protected ListAdapterListModel<T> modelData;
...
//    protected ListModel<T> listModel = new DefaultListModel<T>();
    ...
    protected RowTableModel(List<String> columnNames) {
        this(new ListAdapterListModel<T>(), columnNames);
    }
     ...
     * columns
     */
    protected RowTableModel(ListAdapterListModel<T> modelData, List<String> columnNames) {
     ...
     * @param rowClass the class of row data to be added to the model
     */
    protected RowTableModel(ListAdapterListModel<T> modelData, List<String> columnNames, Class rowClass) {
        setDataAndColumnNames(modelData, columnNames);
        setRowClass(rowClass);
    }
     ...
     * columns
     */
    protected void setDataAndColumnNames(ListAdapterListModel<T> modelData, List<String> columnNames) {
...
//
    ...
    public T getRow(int row) {
        return modelData.getElementAt(row);
    }
    ...
    public void moveRow(int start, int end, int to) {
        ...
        for (int i = start; i < end + 1; i++) {
            temp.add(modelData.getElementAt(i));
        }
    ...
    public final void setListModel(ListAdapterListModel<T> modelData) {
        if (this.modelData != null) {
            this.modelData.removeListDataListener(listModelChangeListener);
        }
        this.modelData = modelData;
        if (modelData != null) {
            modelData.addListDataListener(listModelChangeListener);
        }
    ...
    final public T getRowData(int row) throws Exception {
        if (row < modelData.getSize()) {
            return modelData.getElementAt(row);
        }

        throw new Exception("getRowData()[ rowindex: " + row + " greater than size: " + modelData.getSize() + "]");
    }
}
