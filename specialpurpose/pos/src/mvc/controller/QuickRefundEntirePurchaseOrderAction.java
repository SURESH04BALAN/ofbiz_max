 ...
 * @author siranjeev
 */
public class QuickRefundEntirePurchaseOrderAction extends ScreenAction {
    ...
    public QuickRefundEntirePurchaseOrderAction(javax.swing.JDesktopPane desktopPane, OrderActionInterface orderActInterface) {
        super(ActionType.QUICK_REFUND_ENTIRE_ORDER_ACTION,
                QuickRefundEntirePurchaseOrderAction.nameStr, XuiContainer.getSession(), desktopPane);
        this.orderActionInterface = orderActInterface;
    }

    public QuickRefundEntirePurchaseOrderAction(String name, XuiSession session, javax.swing.JDesktopPane desktopPane, ListAdapterListModel<Order> orderCompListModel) {
        super(ActionType.QUICK_REFUND_ENTIRE_ORDER_ACTION, name, session, desktopPane);
        this.orderCompListModel = orderCompListModel;
        this.session = session;
    }

    public QuickRefundEntirePurchaseOrderAction(ListAdapterListModel<Order> orderCompListModel, XuiSession session) {
    ...
    public boolean quickRefundEntireOrder(Order order) {
        ...
        try {
            returnResp = dispatcher.runSync("quickReturnOrder", UtilMisc.<String, Object>toMap("orderId", order.getOrderId(),
                    "returnHeaderTypeId", "VENDOR_RETURN", "userLogin", session.getUserLogin()));
