...
public class DeviceLoader {
    ...
    public static org.ofbiz.pos.device.impl.Scale scale = null;
    public static DeviceScale deviceScale = null;
    ...
    public static void load(Map<String, ContainerConfig.Container.Property> devices) throws GeneralException {
        ...
        // load the scale
        if (devices.get("Scale") != null) {
            String scaleDevice = (devices.get("Scale")).value;
            if (UtilValidate.isNotEmpty(scaleDevice) && !"[NOT IMPLEMENTED]".equals(scaleDevice)) {
                scale = new org.ofbiz.pos.device.impl.Scale(scaleDevice, -1);
                try {
                    scale.open();
                } catch (jpos.JposException jpe) {
                    Debug.logError(jpe, "JPOS Exception", module);
        ...
        for (int i = 1; i < 10; i++) { // more than 10 cash drawers on a terminal??
            ...
            if (devices.get(idName) != null) {
                ...
                if (UtilValidate.isNotEmpty(cashDrawerDevice) && !"[NOT IMPLEMENTED]".equals(cashDrawerDevice)) {
                    ...
                    try {
                        drawer[i-1].open();
                        Debug.logInfo("opened cash drawer idName: " + idName + " cashDrawerDevice: " + cashDrawerDevice, module);
                    } catch (jpos.JposException jpe) {
                        Debug.logError(jpe, "JPOS Exception", module);
                        throw new GeneralException(jpe.getOrigException());
                    }
