            ...
            if (KeyboardAdaptor.receivers.size() > 0) {
                if (keyCharData.size() > 0 || keyCodeData.size() > 0) {
                    ...
                    /* try {
                     ...
                     throw new Exception("processScanData: " + new String(chars));
                     } catch (Exception e) {
                     Debug.logError(e, module);
                     }
                     */
                    List<KeyboardReceiver> listReceiviers = new ArrayList<KeyboardReceiver>();
                    for (KeyboardReceiver receiver : receivers.keySet()) {
                        ...
                        if (receiverType == ALL_DATA || receiverType == thisDataType) {
                            if (receiver instanceof org.ofbiz.pos.jpos.service.KeyboardService) {
                                listReceiviers.add(receiver);
                            } else {
                                //Debug.logError("receiver value =" + receiver.toString() + "receiverType: " + receiverType + "thisDataType: " + thisDataType, module);

                                //try {
                                //    throw new Exception(new String(chars) + "\n  ");
                                //} catch (Exception e) {
                                //    Debug.logError(e, module);
                                //}
                    ...
                    for (KeyboardReceiver receiver : listReceiviers) {
                        Debug.logError("receiver value =" + receiver.toString(), module);
                        receiver.receiveData(codes, chars);
                    }
