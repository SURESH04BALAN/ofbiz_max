...
//        myInitComponents();
                    ...
                    if (lsm.isSelectionEmpty()) {
                        ...
                        for (int i = minIndex; i <= maxIndex; i++) {
                            if (lsm.isSelectedIndex(i)) {
                                ...
                                Debug.logInfo("Map val selected 1", "module");
                                if (selectedGenericVal != null && childPanelInterface != null) {
                                    try {
                                        Debug.logInfo("Map val selected 2", "module");
                                        //            Party party = new Party(val);
                                        //           party.setGenericValue();
                                        GenericValueObjectInterface partyObj = childPanelInterface.createUIObject(selectedGenericVal);
    ...
    private void initComponents() {
        ...
        detailPanel.setPreferredSize(new java.awt.Dimension(0, 60));
        ...
        getContentPane().add(detailPanel, java.awt.BorderLayout.PAGE_START);
    ...
    private boolean doCellRendering = true;
    ...
    public void setReadOnlyTable(boolean editableTable) {
        this.readOnlyTable = editableTable;
    }

    public boolean isDoCellRendering() {
        return doCellRendering;
    }

    public void setDoCellRendering(boolean doCellRendering) {
        this.doCellRendering = doCellRendering;
    }

    public void setupOrderTableList(List<GenericValue> dataList) {

        GenericValueTableDataModel model = new GenericValueTableDataModel(IdColumnName, readOnlyTable);
//        model.addTableModelListener(new GenericValueDetailTableDialog.GenericValueTableModelListener());
        ...
        for (int i = 0; i < colCount; i++) {
            ...
            if (columnWidths != null) {
                column.setPreferredWidth(columnWidths.get(i)); //third column is bigger                            
            } else {
...
//            panel.pack();
    ...
    public void setColumnWidths(List columnWidth) {
...
//        model.addTableModelListener(new GenericValueDetailTableDialog.GenericValueTableModelListener());
            ...
            if (evt.getType() == TableModelEvent.UPDATE) {
                try {
                    ...
                    if (childPanelInterface != null) {
                        GenericValueObjectInterface partyObj = childPanelInterface.createUIObject(genVal);
                        childPanelInterface.changeUIObject(partyObj);
                        childPanelInterface.setUIFields();
                    }
                } catch (java.text.ParseException ex) {
    ...
    private GenericValuePanelInterfaceOrderMax childPanelInterface = null;

    public GenericValuePanelInterfaceOrderMax getChildPanelInterface() {
        return childPanelInterface;
    }

    public void setChildPanelInterface(GenericValuePanelInterfaceOrderMax childPanelInterface) {
