 ...
 * @author BBS Auctions
 */
public class SimpleProductFeatureGroupPanel extends javax.swing.JPanel implements GenericSaveInterface {

    private JGenericListBoxSelectionModel<ProductFeatureGroup> listSelectionModel = null;
     ...
     */
    public SimpleProductFeatureGroupPanel(ControllerOptions options) {
        initComponents();
        this.options = options;
        List<ProductFeatureGroup> list = new ArrayList<ProductFeatureGroup>();//PosTerminalSingleton.getValueList();
        if (options.contains("EntityName")) {
            entityName = (String) options.get("EntityName");
        }

        listSelectionModel = new JGenericListBoxSelectionModel<ProductFeatureGroup>(list);
        ...
        currentRecord = new ProductFeatureGroup();
        setDialogField();

    }

    ProductFeatureGroup currentRecord = null;

    public void setSelected(int index) {

        if (index < listSelectionModel.dataListModel.getSize()) {
            currentRecord = (ProductFeatureGroup) listSelectionModel.dataListModel.getElementAt(index);
            setDialogField();
        }
    }
    ...
    public void setDialogField() {
        if (currentRecord != null) {
            ...
            txtProductFeatureGroupId.setText(currentRecord.getproductFeatureGroupId());
    ...
    public void getDialogFields() {
        ...
        if (currentRecord != null) {
            Debug.logInfo(" getDialogFields Print 1", SimpleProductFeatureGroupPanel.class.getName());
            currentRecord.setproductFeatureGroupId(txtProductFeatureGroupId.getText());
    ...
    public void copyRecord() {
        if (currentRecord != null) {
            currentRecord.setproductFeatureGroupId("");
    ...
    public boolean isValidValues() {
        ...
        Debug.logInfo(" isValidValues: " + result, SimpleProductFeatureGroupPanel.class.getName());
        if (!result) {
    ...
    public void loadList(org.ofbiz.ordermax.base.ControllerOptions options) {
        ...
        if (UtilValidate.isNotEmpty(entityName)) {
            ...
            for (GenericValue genVal : genList) {
                list.add(new ProductFeatureGroup(genVal));
            }
        }
    ...
    public void setGenericValue(GenericValue val) {
        currentRecord = new ProductFeatureGroup(val);
        setDialogField();
    }
}
