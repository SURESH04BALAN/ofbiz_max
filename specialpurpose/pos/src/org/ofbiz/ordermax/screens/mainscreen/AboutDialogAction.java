 ...
 * @author siranjeev
 */
public class AboutDialogAction extends ScreenAction {
...
//    private ListAdapterListModel<ShipmentReceiptComposite> invoiceCompListModel;
    ...
    public static final String nameStr = "About Dialog";
    final String iconPathStr = "sales.png";
    final String iconPathSmallStr = "salesorder_small.png";

    private XuiSession session;

    public AboutDialogAction(String name, XuiSession session, javax.swing.JDesktopPane desktopPane) {
        super(ActionType.PRINT_PURCHASE_INVOICE_ACTION, name, session, desktopPane);

        this.session = session;
    }

    public AboutDialogAction(ItemListInterface<ShipmentReceiptComposite> itemListInterface, XuiSession session) {
    ...
    protected void setSizeAndLocation(TwoPanelNonSizableContainerDlg dlg) {
        int y = 10;
        int x = 10;
        dlg.setSize(800 - 2 * x, 750 - 2 * y);
        dlg.setLocation(x, y);
    }

    public void actionPerformed(ActionEvent e) {
        AboutPanel viewer = new AboutPanel();
        if (desktopPane != null) {

            JOptionPane pane = new JOptionPane(viewer, JOptionPane.PLAIN_MESSAGE,
                    JOptionPane.DEFAULT_OPTION, null, new Object[0], null);

            final JInternalFrame dialog = pane.createInternalFrame(desktopPane, "About");
