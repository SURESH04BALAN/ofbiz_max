 ...
 * @author siranjeev
 */
public class NotPaidPaymentAction extends ScreenAction {
    ...
    public static final String nameStr = "NOT PAID";
    ...
    public NotPaidPaymentAction(javax.swing.JDesktopPane desktopPane,  PaymentActionInterface paymentActionInterface) {
        super(ActionType.APPROVE_PURCHASE_ORDER_ACTION,
                NotPaidPaymentAction.nameStr, XuiContainer.getSession(), desktopPane);
        this.paymentActionInterface = paymentActionInterface;
    }

    public NotPaidPaymentAction(String name, XuiSession session, javax.swing.JDesktopPane desktopPane, ListAdapterListModel<PaymentComposite> orderCompListModel) {
        super(ActionType.APPROVE_PURCHASE_ORDER_ACTION, name, session, desktopPane);
        this.orderCompListModel = orderCompListModel;
        this.session = session;
    }

    public NotPaidPaymentAction(ListAdapterListModel<PaymentComposite> orderCompListModel, XuiSession session) {
    ...
    public void actionPerformed(ActionEvent e) {
        if (paymentActionInterface != null) {
            ...
            if (notPaidPayment(payment )) {
                ...
                int result = OrderMaxOptionPane.showConfirmDialog(null, "Unable to  Payment: " + payment.getPayment().getpaymentId(), "Not Paid",
     ...
     public boolean notPaidPayment(PaymentComposite payment ) {

         try {
            return LoadPaymentWorker.setStatusToPaymentNotPaid(payment, session) == OrderMaxUtility.ServiceReturnStatus.SUCCESS;
        } catch (Exception ex) {
