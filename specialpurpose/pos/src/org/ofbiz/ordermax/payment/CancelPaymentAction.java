 ...
 * @author siranjeev
 */
public class CancelPaymentAction extends ScreenAction {
    ...
    public static final String nameStr = "CANCEL PAYMENT";
    ...
    public CancelPaymentAction(javax.swing.JDesktopPane desktopPane,  PaymentActionInterface paymentActionInterface) {
        super(ActionType.APPROVE_PURCHASE_ORDER_ACTION,
                CancelPaymentAction.nameStr, XuiContainer.getSession(), desktopPane);
        this.paymentActionInterface = paymentActionInterface;
    }

    public CancelPaymentAction(String name, XuiSession session, javax.swing.JDesktopPane desktopPane, ListAdapterListModel<Order> orderCompListModel) {
        super(ActionType.APPROVE_PURCHASE_ORDER_ACTION, name, session, desktopPane);
        this.orderCompListModel = orderCompListModel;
        this.session = session;
    }

    public CancelPaymentAction(ListAdapterListModel<Order> orderCompListModel, XuiSession session) {
    ...
    public void actionPerformed(ActionEvent e) {
        if (paymentActionInterface != null) {
            ...
            if (cancelPayment(payment )) {
                ...
                int result = OrderMaxOptionPane.showConfirmDialog(null, "Unable to Cancel Payment: " + payment.getPayment().getpaymentId(), "Cancel Payment",
     ...
     public boolean cancelPayment(PaymentComposite payment ) {

         try {
            return LoadPaymentWorker.setStatusToPaymentCancelled(payment, session) == OrderMaxUtility.ServiceReturnStatus.SUCCESS;
        } catch (Exception ex) {
