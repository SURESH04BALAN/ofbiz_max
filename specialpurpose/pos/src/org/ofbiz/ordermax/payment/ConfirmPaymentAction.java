 ...
 * @author siranjeev
 */
public class ConfirmPaymentAction extends ScreenAction {
    ...
    public static final String nameStr = "CONFIRM PAYMENT";
    ...
    public ConfirmPaymentAction(javax.swing.JDesktopPane desktopPane,  PaymentActionInterface paymentActionInterface) {
        super(ActionType.APPROVE_PURCHASE_ORDER_ACTION,
                ConfirmPaymentAction.nameStr, XuiContainer.getSession(), desktopPane);
        this.paymentActionInterface = paymentActionInterface;
    }

    public ConfirmPaymentAction(String name, XuiSession session, javax.swing.JDesktopPane desktopPane, ListAdapterListModel<Order> orderCompListModel) {
        super(ActionType.APPROVE_PURCHASE_ORDER_ACTION, name, session, desktopPane);
        this.orderCompListModel = orderCompListModel;
        this.session = session;
    }

    public ConfirmPaymentAction(ListAdapterListModel<Order> orderCompListModel, XuiSession session) {
    ...
    public void actionPerformed(ActionEvent e) {
        if (paymentActionInterface != null) {
            ...
            if (confirmPayment(payment )) {
                ...
                int result = OrderMaxOptionPane.showConfirmDialog(null, "Unable to Confirm Payment: " + payment.getPayment().getpaymentId(), "Confirm Payment",
     ...
     public boolean confirmPayment(PaymentComposite payment ) {

         try {
            return LoadPaymentWorker.setStatusToPaymentConfirmed(payment, session) == OrderMaxUtility.ServiceReturnStatus.SUCCESS;
        } catch (Exception ex) {
