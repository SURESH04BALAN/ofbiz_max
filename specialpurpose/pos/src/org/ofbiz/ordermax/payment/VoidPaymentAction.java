 ...
 * @author siranjeev
 */
public class VoidPaymentAction extends ScreenAction {
    ...
    public static final String nameStr = "VOID PAYMENT";
    ...
    public VoidPaymentAction(javax.swing.JDesktopPane desktopPane,  PaymentActionInterface paymentActionInterface) {
        super(ActionType.APPROVE_PURCHASE_ORDER_ACTION,
                VoidPaymentAction.nameStr, XuiContainer.getSession(), desktopPane);
        this.paymentActionInterface = paymentActionInterface;
    }

    public VoidPaymentAction(String name, XuiSession session, javax.swing.JDesktopPane desktopPane, ListAdapterListModel<PaymentComposite> orderCompListModel) {
        super(ActionType.APPROVE_PURCHASE_ORDER_ACTION, name, session, desktopPane);
        this.orderCompListModel = orderCompListModel;
        this.session = session;
    }

    public VoidPaymentAction(ListAdapterListModel<PaymentComposite> orderCompListModel, XuiSession session) {
    ...
    public void actionPerformed(ActionEvent e) {
        if (paymentActionInterface != null) {
            ...
            if (voidPayment(payment )) {
                ...
                int result = OrderMaxOptionPane.showConfirmDialog(null, "Unable to Void Payment: " + payment.getPayment().getpaymentId(), "Void Payment",
     ...
     public boolean voidPayment(PaymentComposite payment ) {

         try {
            return LoadPaymentWorker.voidPayment(payment, session) == OrderMaxUtility.ServiceReturnStatus.SUCCESS;
        } catch (Exception ex) {
