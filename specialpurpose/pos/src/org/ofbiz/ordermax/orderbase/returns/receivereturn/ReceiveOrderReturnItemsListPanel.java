 ...
 * @author siranjeev
 */
public class ReceiveOrderReturnItemsListPanel extends javax.swing.JPanel{
...
/*
     ...
     */
    public ReceiveOrderReturnItemsListPanel(ControllerOptions options) {
        initComponents();
...
//        receiveOrderReturnItemTableModel = new ReceiveOrderReturnItemTableModel();
//        ControllerOptions orderOptions = new ControllerOptions(options);
        ...
        receiveOrderReturnItemTableModel.addTableModelListener(new InteractiveTableModelListener());
...
//        tableReturnItems.setSelectAllForEdit(true);
        ...
        tableReturnItems.setModel(receiveOrderReturnItemTableModel);
...
//        btnToPartyId.addActionListener(new LookupActionListner(txtToPartyId, "partyIdTextField", "BILL_FROM_VENDOR", null));        
    ...
    final public void setupEditOrderTable() {
        ...
        int colCount = receiveOrderReturnItemTableModel.getColumnCount();
        ReceiveOrderReturnItemTableModel.Columns[] columns = ReceiveOrderReturnItemTableModel.Columns.values();

        for (int i = 0; i < colCount; i++) {
            TableColumn column = tableReturnItems.getColumnModel().getColumn(i);
            ReceiveOrderReturnItemTableModel.Columns columnValues = columns[i];
            column.setPreferredWidth(columnValues.getColumnWidth());
            ReceiveOrderReturnItemTableModel.Columns columnVal = columns[i];
            if (i == ReceiveOrderReturnItemTableModel.Columns.ORDER_PROD_ID_INDEX.getColumnIndex()) {
                column.setCellEditor(productTreeActionTableCellEditor);
            } else if (i == ReceiveOrderReturnItemTableModel.Columns.INVENTORY_LOCATION.getColumnIndex()) {

                TableColumn tmpColum = column;
                setUpSportColumn(tableReturnItems, tmpColum);
            } else if (i == ReceiveOrderReturnItemTableModel.Columns.INVENTORY_ITEM_STATUS_INDEX.getColumnIndex()) {
                TableColumn tmpColum = column;
                setUpStatusColumn(tableReturnItems, tmpColum);
            } else if (i == ReceiveOrderReturnItemTableModel.Columns.INVENTORY_SERIALIZED.getColumnIndex()) {
                TableColumn tmpColum = column;
    ...
    private void initColumnSizes(JTable table) {
        /*    ReceiveOrderReturnItemTableModel model = (ReceiveOrderReturnItemTableModel)table.getModel();
...
//        lblReturnId.setText(getReturnHeaderComposite().getReturnHeader().getreturnId());
    ...
    private ReceiveOrderReturnItemTableModel receiveOrderReturnItemTableModel = new ReceiveOrderReturnItemTableModel();

    public ReceiveOrderReturnItemTableModel getReceiveOrderReturnItemTableModel() {
        return receiveOrderReturnItemTableModel;
    }
...
//        java.awt.Dimension dim = tableReturnItems.getPreferredSize();
        ...
        receiveOrderReturnItemTableModel.setListModel(cutdownList);
        tableReturnItems.setModel(receiveOrderReturnItemTableModel);
    ...
    public class InteractiveTableModelListener implements TableModelListener {
        ...
        public void tableChanged(TableModelEvent evt) {
            if (evt.getType() == TableModelEvent.UPDATE) {
                ...
                if (column + 1 < receiveOrderReturnItemTableModel.getColumnCount()) {
                    tableReturnItems.setColumnSelectionInterval(column + 1, column + 1);
                }
                if (row < receiveOrderReturnItemTableModel.getRowCount()) {
