 ...
 * @author siranjeev
 */
public class UomDataSpeedSingleton implements Serializable {

    private static final long serialVersionUID = 1L;
    Map<String, Uom> valueMap = null;

    private UomDataSpeedSingleton() {
        valueMap = new HashMap<String, Uom>();
    }

    private static class SingletonHolder {

        public static final UomDataSpeedSingleton INSTANCE = new UomDataSpeedSingleton();
    }

    public static UomDataSpeedSingleton getInstance() {
    ...
    static protected void loadAll() {
        try {
            ...
            whereClauseMap.put("uomTypeId", "DATASPD_MEASURE");
            List<GenericValue> valueList = PosProductHelper.getGenericValueListsWithSelection("Uom", whereClauseMap, "description", UomDataSpeedSingleton.singletonSession.getDelegator());
            for (GenericValue val : valueList) {
                Uom uom = UOMSingleton.getUom(val.getString("uomId"));//new Uom(val);
                getInstance().valueMap.put(uom.getuomId(), uom);
            }
        } catch (Exception ex) {
            Logger.getLogger(UomDataSpeedSingleton.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    public static void setSingletonSession(XuiSession singletonSession) {
        UomDataSpeedSingleton.singletonSession = singletonSession;
    }
}
