 ...
 * @author siranjeev
 */
public class UomTemperatureSingleton implements Serializable {

    private static final long serialVersionUID = 1L;
    Map<String, Uom> valueMap = null;

    private UomTemperatureSingleton() {
        valueMap = new HashMap<String, Uom>();
    }

    private static class SingletonHolder {

        public static final UomTemperatureSingleton INSTANCE = new UomTemperatureSingleton();
    }

    public static UomTemperatureSingleton getInstance() {
    ...
    static protected void loadAll() {
        try {
            ...
            whereClauseMap.put("uomTypeId", "TEMP_MEASURE");
            List<GenericValue> valueList = PosProductHelper.getGenericValueListsWithSelection("Uom", whereClauseMap, "description", UomTemperatureSingleton.singletonSession.getDelegator());
            for (GenericValue val : valueList) {
                Uom uom = UOMSingleton.getUom(val.getString("uomId"));//new Uom(val);
                getInstance().valueMap.put(uom.getuomId(), uom);
            }
        } catch (Exception ex) {
            Logger.getLogger(UomTemperatureSingleton.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    public static void setSingletonSession(XuiSession singletonSession) {
        UomTemperatureSingleton.singletonSession = singletonSession;
    }
}
