 ...
 * @author siranjeev
 */
public class CategoryCopyPanelFrame extends javax.swing.JPanel {

    public static final String module = CategoryCopyPanelFrame.class.getName();
    /**
...
//    ProductCategoryPanel productCategoryPanel = null;
    ...
    public JListBoxSelectionModel<TreeNode> productListSelectionModel = null;

    public ListAdapterListModel<TreeNode> getModel() {
        return productListSelectionModel.dataListModel;
    }

    ControllerOptions options = null;

    public CategoryCopyPanelFrame(ControllerOptions options) {
        initComponents();
...
//        jlistTreeSelect.setModel(prevMovel);
        ...
        productListSelectionModel = new JListBoxSelectionModel<TreeNode>(new CategoryTreeSelectionPanelFrame.TreeNodeListCellRenderer());
...
//                        System.exit(0);
    ...
    private void btnAddActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAddActionPerformed
        ...
        ListAdapterListModel<TreeNode> prevMovel = getModel();        // TODO add your handling code here:
        int index = productListSelectionModel.jlistBox.getSelectedIndex();
        Debug.logError("getSelectedIndex: " + index, module);
        TreeNode productNode = productListSelectionModel.listModelSelection.getSelection();//.productListSelectionModel.selectionModel.getLeadSelectionIndex();.getSelectedValue();
        if (productNode == null) {
        ...
        prevMovel.remove((TreeNode) productListSelectionModel.jlistBox.getSelectedValue());
...
//            reloadTreePanel(categoryId);
    ...
    public void setSeletedProducts(ListAdapterListModel<TreeNode> seleProductList) {

        for (int i = 0; i < seleProductList.getSize(); i++) {
            TreeNode node = seleProductList.getElementAt(i);
            productListSelectionModel.dataListModel.add(node);
            System.out.println("TreeNode parent_catid: " + node._id);
        }
    }

    public void setSeletedProducts(TreeNode product) {
        productListSelectionModel.dataListModel.clear();
        productListSelectionModel.dataListModel.add(product);
        System.out.println("TreeNode parent_catid: " + product._id);
    }

    void copyItem(TreeNode catNode, TreeNode parentCatNode) {
        GenericValue category = PosProductHelper.getProductCategory(catNode._id, ControllerOptions.getSession().getDelegator());

        if (category != null) {
            GenericValue rollUp = PosProductHelper.getProductCategoryRollup(catNode._id, parentCatNode._id, ControllerOptions.getSession().getDelegator());
            if (rollUp == null) {
                try {
                    rollUp = PosProductHelper.createProductCategoryRollup(catNode._id, parentCatNode._id, ControllerOptions.getSession().getDelegator());
                    ControllerOptions.getSession().getDelegator().create(rollUp);
                } catch (GenericEntityException ex) {
                    Logger.getLogger(CategoryCopyPanelFrame.class.getName()).log(Level.SEVERE, null, ex);
                }
            }
        }
...
//            DefaultListModel prevMovel = (DefaultListModel) jlistTreeSelect.getModel();        // TODO add your handling code here:
            List<TreeNode> selList = productTreePanel.getAllSelectedTreeNode(ProductCategoryTreeNode.ProductCategoryTreeNodeName);
            if (selList != null) {
                try {
                    for (int i = 0; i < prevMovel.getSize(); ++i) {
                        TreeNode productNode = prevMovel.getElementAt(i);
                        copyItem(productNode, destNode);
                    }
                    prevMovel.clear();
                    productListSelectionModel.jlistBox.setModel(prevMovel);
    ...
    private void btnRemoveActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnRemoveActionPerformed
        ...
        if (tp != null) {
            ...
            if (productNode != null) {
                ...
                System.out.print(" ProductCategoryTreeNode.ProdutNodeName      " + ProductCategoryTreeNode.ProductCategoryTreeNodeName);

                if (!productNode.getNodeName().equals(ProductCategoryTreeNode.ProductCategoryTreeNodeName)) {
                    System.out.print(" return: " + productNode.getNodeName().equals(ProductCategoryTreeNode.ProductCategoryTreeNodeName));
                    return;
                }
    ...
    private void btnRemoveAllActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnRemoveAllActionPerformed
        ...
        if (tp != null) {
            ...
            if (catNode != null) {
                ...
                List<TreeNode> selList = productTreePanel.getAllChildTreeNode();//getAllSelectedTreeNode(ProductCategoryTreeNode.ProdutNodeName);
                if (selList != null) {
                    try {
