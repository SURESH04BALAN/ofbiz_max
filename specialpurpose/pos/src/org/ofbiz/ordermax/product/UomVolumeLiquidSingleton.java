 ...
 * @author siranjeev
 */
public class UomVolumeLiquidSingleton implements Serializable {

    private static final long serialVersionUID = 1L;
    Map<String, Uom> valueMap = null;

    private UomVolumeLiquidSingleton() {
        valueMap = new HashMap<String, Uom>();
    }

    private static class SingletonHolder {

        public static final UomVolumeLiquidSingleton INSTANCE = new UomVolumeLiquidSingleton();
    }

    public static UomVolumeLiquidSingleton getInstance() {
    ...
    static protected void loadAll() {
        try {
            ...
            whereClauseMap.put("uomTypeId", "LENGTH_MEASURE");
            List<GenericValue> valueList = PosProductHelper.getGenericValueListsWithSelection("Uom", whereClauseMap, "description", UomVolumeLiquidSingleton.singletonSession.getDelegator());
            for (GenericValue val : valueList) {
                Uom uom = UOMSingleton.getUom(val.getString("uomId"));//new Uom(val);
                getInstance().valueMap.put(uom.getuomId(), uom);
            }
        } catch (Exception ex) {
            Logger.getLogger(UomVolumeLiquidSingleton.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    public static void setSingletonSession(XuiSession singletonSession) {
        UomVolumeLiquidSingleton.singletonSession = singletonSession;
    }
}
