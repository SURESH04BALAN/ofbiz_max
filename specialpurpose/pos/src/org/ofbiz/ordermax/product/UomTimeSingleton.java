 ...
 * @author siranjeev
 */
public class UomTimeSingleton implements Serializable {

    private static final long serialVersionUID = 1L;
    Map<String, Uom> valueMap = null;

    private UomTimeSingleton() {
        valueMap = new HashMap<String, Uom>();
    }

    private static class SingletonHolder {

        public static final UomTimeSingleton INSTANCE = new UomTimeSingleton();
    }

    public static UomTimeSingleton getInstance() {
    ...
    static protected void loadAll() {
        try {
            ...
            whereClauseMap.put("uomTypeId", "TIME_FREQ_MEASURE");
            List<GenericValue> valueList = PosProductHelper.getGenericValueListsWithSelection("Uom", whereClauseMap, "description", UomTimeSingleton.singletonSession.getDelegator());
            for (GenericValue val : valueList) {
                Uom uom = UOMSingleton.getUom(val.getString("uomId"));//new Uom(val);
                getInstance().valueMap.put(uom.getuomId(), uom);
            }
        } catch (Exception ex) {
            Logger.getLogger(UomTimeSingleton.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    public static void setSingletonSession(XuiSession singletonSession) {
        UomTimeSingleton.singletonSession = singletonSession;
    }
}
