 ...
 * @author siranjeev
 */
public class UomDataMeasureSingleton implements Serializable {

    private static final long serialVersionUID = 1L;
    Map<String, Uom> valueMap = null;

    private UomDataMeasureSingleton() {
        valueMap = new HashMap<String, Uom>();
    }

    private static class SingletonHolder {

        public static final UomDataMeasureSingleton INSTANCE = new UomDataMeasureSingleton();
    }

    public static UomDataMeasureSingleton getInstance() {
    ...
    static protected void loadAll() {
        try {
            ...
            whereClauseMap.put("uomTypeId", "DATA_MEASURE");
            List<GenericValue> valueList = PosProductHelper.getGenericValueListsWithSelection("Uom", whereClauseMap, "description", UomDataMeasureSingleton.singletonSession.getDelegator());
            for (GenericValue val : valueList) {
                Uom uom = UOMSingleton.getUom(val.getString("uomId"));//new Uom(val);
                getInstance().valueMap.put(uom.getuomId(), uom);
            }
        } catch (Exception ex) {
            Logger.getLogger(UomDataMeasureSingleton.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    public static void setSingletonSession(XuiSession singletonSession) {
        UomDataMeasureSingleton.singletonSession = singletonSession;
    }
}
