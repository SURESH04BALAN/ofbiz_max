 ...
 * @author siranjeev
 */
public class UomLenghtSingleton implements Serializable {

    private static final long serialVersionUID = 1L;
    Map<String, Uom> valueMap = null;

    private UomLenghtSingleton() {
        valueMap = new HashMap<String, Uom>();
    }

    private static class SingletonHolder {

        public static final UomLenghtSingleton INSTANCE = new UomLenghtSingleton();
    }

    public static UomLenghtSingleton getInstance() {
    ...
    static protected void loadAll() {
        try {
            ...
            List<GenericValue> valueList = PosProductHelper.getGenericValueListsWithSelection("Uom", whereClauseMap, "description", XuiContainer.getSession().getDelegator());
            for (GenericValue val : valueList) {
                Uom uom = UOMSingleton.getUom(val.getString("uomId"));//new Uom(val);
                getInstance().valueMap.put(uom.getuomId(), uom);
            }
        } catch (Exception ex) {
            Logger.getLogger(UomLenghtSingleton.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
