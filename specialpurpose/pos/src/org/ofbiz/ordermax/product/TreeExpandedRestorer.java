...
public class TreeExpandedRestorer {
    ...
    /**
     * Selections are stored with path information. If you plan to insert
     * different nodes, you should use this option.
     */
    public static final SelectionType SELECTION_BY_PATH = new SelectionType("SelectionByPath");
    /**
     * Selections are stored with row information. If you decide to delete
     * nodes, you should select this option.
     */
     ...
     * @param selectionType the type of selection, either
     * {@link #SELECTION_BY_ROW} or {@link #SELECTION_BY_PATH}
     */
    ...
    /**
     * Saves the expanded state and optionally discards the selection of the
     * tree.
     *
     * @param tree the tree for which selection and expanstion state are saved
    ...
    /**
     * Use this method if you want to transfer a node with a tree, and you want
     * to have the selection transfered too.
     *
     * @param target the node being transfered
     * @param old the source tree path where the node was or is attached to
     * @param nw the destination tree path where the node will be or is attached
     * to
     */
    public void update(org.ofbiz.ordermax.base.TreeNode target, TreePath old, TreePath nw) {
...
//                selectionRows = ArrayUtils.remove(selectionRows, j);
    ...
    private void saveExpandedState() {
        ...
        TreePath rootPath = new TreePath(((org.ofbiz.ordermax.base.TreeNode) tree.getModel().getRoot()));

        Enumeration enum1 = tree.getExpandedDescendants(rootPath);

        while (enum1 != null && enum1.hasMoreElements()) {

            expandedPaths.add(enum1.nextElement());
