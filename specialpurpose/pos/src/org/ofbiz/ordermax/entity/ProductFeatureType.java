...
public class ProductFeatureType implements GenericValueObjectInterface , DisplayNameInterface{

    public static final String module = ProductFeatureType.class.getName();
    public static int COL_SIZE = 80;
    final static String Default_Cell_Rendere = "DEFAULT";
    final static String GenericValue_Cell_Renderer = "GENERIC_VALUE";
    private GenericValue genVal = null;

    public ProductFeatureType(GenericValue val) {
        genVal = val;
        try {
            setGenericValue();
        } catch (Exception ex) {
    ...
    public org.ofbiz.entity.GenericValue createNewGenericValueObj(org.ofbiz.entity.Delegator delegator) {
        genVal = delegator.makeValue("ProductFeatureType");
        getGenericValue(genVal);
        return genVal;
    }
    ...
    public Collection getObjectList(List<org.ofbiz.entity.GenericValue> genList) {
        Collection<ProductFeatureType> objectList = new ArrayList<ProductFeatureType>();
        for (GenericValue genVal : genList) {
            objectList.add(new ProductFeatureType(genVal));
        }
        return objectList;
    }
    ...
    public String getdisplayName(DisplayTypes showKey) {
        ...
        if (DisplayTypes.SHOW_NAME_AND_CODE.equals(showKey) || DisplayTypes.SHOW_NAME_ONLY.equals(showKey)) {
            ...
            if (DisplayTypes.SHOW_NAME_AND_CODE.equals(showKey)) {
                ...
                orderToStringBuilder.append(getproductFeatureTypeId());
                orderToStringBuilder.append(" ]");
            }
        } else {
            orderToStringBuilder.append(getproductFeatureTypeId());
