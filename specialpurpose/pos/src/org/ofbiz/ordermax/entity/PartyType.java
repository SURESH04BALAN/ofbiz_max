...
public class PartyType implements GenericValueObjectInterface, DisplayNameInterface {

    public static final String module = PartyType.class.getName();
    public static int COL_SIZE = 80;
    final static String Default_Cell_Rendere = "DEFAULT";
    final static String GenericValue_Cell_Renderer = "GENERIC_VALUE";
    private GenericValue genVal = null;

    public PartyType(GenericValue val) {
        genVal = val;
        try {
            setGenericValue();
        } catch (Exception ex) {
    ...
    public org.ofbiz.entity.GenericValue createNewGenericValueObj(org.ofbiz.entity.Delegator delegator) {
        genVal = delegator.makeValue("PartyType");
        getGenericValue(genVal);
        return genVal;
    }
    ...
    public Collection getObjectList(List<org.ofbiz.entity.GenericValue> genList) {
        Collection<PartyType> objectList = new ArrayList<PartyType>();
        for (GenericValue genVal : genList) {
            objectList.add(new PartyType(genVal));
        }
        return objectList;
    }
    ...
    public String getdisplayName(DisplayNameInterface.DisplayTypes showId) {
        ...
        if (DisplayNameInterface.DisplayTypes.SHOW_NAME_AND_CODE.equals(showId) || DisplayNameInterface.DisplayTypes.SHOW_NAME_ONLY.equals(showId)) {
            ...
            if (DisplayNameInterface.DisplayTypes.SHOW_NAME_AND_CODE.equals(showId)) {
                ...
                orderToStringBuilder.append(getpartyTypeId());
                orderToStringBuilder.append(" ]");
            }
        } else {
            orderToStringBuilder.append(getpartyTypeId());
