...
public class PaymentGroupType implements GenericValueObjectInterface , DisplayNameInterface {

    public static final String module = PaymentGroupType.class.getName();
    public static int COL_SIZE = 80;
    final static String Default_Cell_Rendere = "DEFAULT";
    final static String GenericValue_Cell_Renderer = "GENERIC_VALUE";
    private GenericValue genVal = null;

    public PaymentGroupType(GenericValue val) {
        genVal = val;
        try {
            setGenericValue();
        } catch (Exception ex) {
    ...
    public org.ofbiz.entity.GenericValue createNewGenericValueObj(org.ofbiz.entity.Delegator delegator) {
        genVal = delegator.makeValue("PaymentGroupType");
        getGenericValue(genVal);
        return genVal;
    }
    ...
    public Collection getObjectList(List<org.ofbiz.entity.GenericValue> genList) {
        Collection<PaymentGroupType> objectList = new ArrayList<PaymentGroupType>();
        for (GenericValue genVal : genList) {
            objectList.add(new PaymentGroupType(genVal));
        }
        return objectList;
    }
    ...
    public String getdisplayName(DisplayTypes showId) {
        ...
        if (DisplayTypes.SHOW_NAME_AND_CODE.equals(showId) || DisplayTypes.SHOW_NAME_ONLY.equals(showId)) {
            ...
            if (DisplayTypes.SHOW_NAME_AND_CODE.equals(showId)) {
                ...
                orderToStringBuilder.append(paymentGroupTypeId);
                orderToStringBuilder.append(" ]");
            }
        } else {
            orderToStringBuilder.append(paymentGroupTypeId);
