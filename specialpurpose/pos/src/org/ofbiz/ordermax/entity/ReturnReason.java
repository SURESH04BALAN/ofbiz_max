...
public class ReturnReason implements GenericValueObjectInterface, DisplayNameInterface {

    public static final String module = ReturnReason.class.getName();
    public static int COL_SIZE = 80;
    final static String Default_Cell_Rendere = "DEFAULT";
    final static String GenericValue_Cell_Renderer = "GENERIC_VALUE";
    private GenericValue genVal = null;

    public ReturnReason(GenericValue val) {
        genVal = val;
        try {
            setGenericValue();
        } catch (Exception ex) {
    ...
    public org.ofbiz.entity.GenericValue createNewGenericValueObj(org.ofbiz.entity.Delegator delegator) {
        genVal = delegator.makeValue("ReturnReason");
        getGenericValue(genVal);
        return genVal;
    }
    ...
    public Collection getObjectList(List<org.ofbiz.entity.GenericValue> genList) {
        Collection<ReturnReason> objectList = new ArrayList<ReturnReason>();
        for (GenericValue genVal : genList) {
            objectList.add(new ReturnReason(genVal));
        }
        return objectList;
    }
    ...
    public String getdisplayName(DisplayNameInterface.DisplayTypes showKey) {
        ...
        if (DisplayNameInterface.DisplayTypes.SHOW_NAME_AND_CODE.equals(showKey) || DisplayNameInterface.DisplayTypes.SHOW_NAME_ONLY.equals(showKey)) {
            ...
            if (DisplayNameInterface.DisplayTypes.SHOW_NAME_AND_CODE.equals(showKey)) {
                ...
                orderToStringBuilder.append(getreturnReasonId());
                orderToStringBuilder.append(" ]");
            }
        } else {
            orderToStringBuilder.append(getreturnReasonId());
