...
public class ImportProductHelper {
    ...
    public static Map<String, Object> prepareInventoryItem(String productId,
        ...
        fields.put("productId", productId);
        fields.put("ownerPartyId", ownerPartyId);
    ...
    public static boolean checkProductExists(String productId,
        ...
        try {
            ...
            if (tmpProductGV != null && productId.equals(tmpProductGV.getString("productId"))){
                ...
                Debug.logInfo("Product Exists [productId]: " + tmpProductGV.getString("productId"), module);                
            }
    ...
    // check if product already exists in database
    public static boolean checkPartyExists(String partyId,
            Delegator delegator) {
    	        
        boolean partyExists = false;
        try {
        	GenericValue party = delegator.findByPrimaryKeyCache("Party", UtilMisc.toMap("partyId", partyId));        	
        	partyExists = !UtilValidate.isEmpty(party);
        	if(partyExists)
        		Debug.logInfo("Product Exists [productId]: " + party.getString("productId"), module);        	
