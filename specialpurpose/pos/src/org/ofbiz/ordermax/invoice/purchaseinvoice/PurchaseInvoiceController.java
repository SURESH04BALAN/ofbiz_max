 ...
 * @author siranjeev
 */
public class PurchaseInvoiceController extends BaseMainScreen {

    public static final String module = PurchaseInvoiceController.class.getName();
    public PurchaseInvoiceEnteryPanel panel = null;
    public final String caption = "Purchase Invoice";
    public PosScreen pos = null;
    String invoiceId = null;

    public PurchaseInvoiceController(XuiSession sess) {
        super(sess);

    }

    public PurchaseInvoiceController(String invoiceId, XuiSession sess) {
        super(sess);
    ...
    public void loadScreen(final ContainerPanelInterface contFrame) {
        ...
        panel = new PurchaseInvoiceEnteryPanel(ControllerOptions.getSession());
        ...
        panel.btnOrderLookup.addActionListener(new LookupActionListner(panel.orderIdTextField, "orderIdTextField", panel.partyIdTextField, "partyIdTextField"));
                ...
                if (e.getSource() instanceof PartyIdVerifyValidator) {
                    try {
                        ...
                        if (validator.getField() != null && validator.getField().getText().isEmpty() == false) {
                            ...
                            if (panel.getOrder() != null && panel.getOrder().getPartyId().equals(billFromPartyId)) {
                                return;
                            } else {
...
//                    panel.loadItemEditDataModel();  //
    ...
    public void newOrder(String billFromPartyId, String billToPartyId) {
        try {
            ...
            InvoiceComposite order = LoadInvoiceWorker.newInvoice(ControllerOptions.getSession(), "PURCHASE_INVOICE", billFromAccount, billToAccount);
...
//        SupplierProductAndListPriceData data = PosProductHelper.getOrderItemDetails(prodId, order.getDestinationFacilityId(), ControllerOptions.getSession());
        ...
        if (orderItem.getShoppingCartItem() == null) {
            /*sur            int index = order.getShopingCart().addItemToEnd(
