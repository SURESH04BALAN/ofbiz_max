...
/*
            ...
            ScreenStringRenderer screenStringRenderer1 = new MacroScreenRenderer(UtilProperties.getPropertyValue("widget", getName() + ".name"), UtilProperties.getPropertyValue("widget", getName() + ".screenrenderer"));
            FormStringRenderer formStringRenderer1 = new MacroFormRenderer(UtilProperties.getPropertyValue("widget", getName() + ".formrenderer"), request, response);
            TreeStringRenderer treeStringRenderer1 = new MacroTreeRenderer(UtilProperties.getPropertyValue("widget", getName() + ".treerenderer"), writer1);
            // TODO: uncomment these lines when the renderers are implemented
            //MenuStringRenderer menuStringRenderer = new MacroMenuRenderer(UtilProperties.getPropertyValue("widget", getName() + ".menurenderer"), writer);

            ScreenRenderer screens1 = new ScreenRenderer(writer1, null, screenStringRenderer);
            screens1.populateContextForRequest(request, response, servletContext);
            // this is the object used to render forms from their definitions
            screens1.getContext().put("formStringRenderer", formStringRenderer1);
            screens1.getContext().put("treeStringRenderer", treeStringRenderer1);
            //screens.getContext().put("menuStringRenderer", menuStringRenderer);
            screens1.getContext().put("simpleEncoder", StringUtil.getEncoder(UtilProperties.getPropertyValue("widget", getName() + ".encoder")));
            screenStringRenderer1.renderScreenBegin(writer1, screens1.getContext());
            screens1.render(page);
            screenStringRenderer1.renderScreenEnd(writer1, screens1.getContext());
...
//            Debug.logInfo("writer: " + stringReadFromReader, module);
            File file = new File("test.html");
            //try (
            BufferedWriter bwwriter = new BufferedWriter(new FileWriter(file));
            bwwriter.write(builder.toString());
            bwwriter.flush();
            bwwriter.close();
